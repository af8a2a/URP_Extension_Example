...
namespace URP_Extension.Features.ScreenSpaceRaytracing
{
    ...
    {
        [InspectorName("State (Opaque)"),
         Tooltip(
             "When set to Enabled, URP processes SSR on opaque objects for Cameras in the influence of this effect's Volume.")]
        ...
        [InspectorName("Minimum Smoothness"),
         Tooltip("SSR ignores a pixel if its smoothness value is lower than this value.")]
        ...
        [InspectorName("Smoothness Fade Start"),
         Tooltip("Use the slider to set the smoothness value at which SSR reflections begin to fade out.")]
        ...
        [InspectorName("Screen Edge Fade Distance"),
         Tooltip("Fades out screen space reflection when it is near the screen boundaries.")]
        ...
        [InspectorName("Object Thickness"),
         Tooltip(
             "The thickness of all scene objects. This is also the fallback thickness for automatic thickness mode.")]
        ...
        [InspectorName("Accumulation Factor"),
         Tooltip(
             "The speed of accumulation convergence for PBR Accumulation mode. Does not work properly with distortion post-processing effects due to URP limitations.")]
        public ClampedFloatParameter accumFactor = new(value: 0.75f, min: 0.0f, max: 1.0f, overrideState: false);


        [Header("Performance")] [Tooltip("The resolution of screen space ray marching.")]
        ...
        public MipmapsModeParameter mipmapsMode = new MipmapsModeParameter(MipmapsMode.Trilinear);
        ...
        public enum Algorithm
        {
            ...
            [InspectorName("PBR Accumulation"),
             Tooltip("Cast rays in stochastic directions and accumulate multiple frames to compute rough reflections.")]
            PBRAccumulation = 1
        }
        ...
        public enum ThicknessMode
        {
            ...
            [InspectorName("Automatic"),
             Tooltip("Automatic mode renders the back-faces of scene objects to compute thickness.")]
            ComputeBackface = 1
        }
        ...
        {
            public ResolutionParameter(Resolution value, bool overrideState = false) : base(value, overrideState)
            {
            }
        }
        ...
        [Serializable]
        public sealed class MipmapsModeParameter : VolumeParameter<MipmapsMode>
        {
            public MipmapsModeParameter(MipmapsMode value, bool overrideState = false) : base(value, overrideState)
            {
            }
        }
    }
