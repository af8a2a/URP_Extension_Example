...
namespace URP_Extension.Features.ScreenSpaceRaytracing.ScreenSpacePathTracing
{
    public class ScreenSpacePathTracing : VolumeComponent, IPostProcessComponent
    {
        ...
        /// </summary>
        [Tooltip(
            "Defines the maximum luminance computed for path tracing. Lower values help prevent noise and fireflies (very bright pixels), but introduce bias by darkening the overall result. Increase this value if your image looks too dark.")]
        ...
        [Header("Ray Marching"),
         Tooltip(
             "Defines the maximum number of steps a path can take during one bounce, affecting the performance of path tracing.")]
        ...
        /// </summary>
        [Tooltip(
            "Defines the initial distance (in meters) covered by each large step, affecting the precision of path tracing.")]
        ...
        [Header("Accumulation"),
         Tooltip(
             "Specifies the denoiser used for screen space path tracing. Enter play mode to apply any real-time denoiser.")]
        ...
        [InspectorName("Accumulation Factor"),
         Tooltip("The speed of accumulation convergence for Temporal and Spatial-Temporal denoiser.")]
        ...
        [Header("Hidden Scene Ambient Settings"), HideInInspector]
        public BoolParameter ambientStored = new BoolParameter(false, overrideState: false);

        [HideInInspector]
        public NoInterpClampedFloatParameter ambientIntensity =
            new(value: 1.0f, min: 0.0f, max: 8.0f, overrideState: false);
        ...
        public enum DenoiserType
        {
            ...
            [Tooltip("No denoising applied.")] None,
            ...
            [InspectorName("Offline"),
             Tooltip("Accumulates over multiple frames and restarts the path accumulation when camera moves.")]
            Offline,
            ...
            [InspectorName("Spatial-Temporal"),
             Tooltip("Combines spatial filtering and temporal re-projection for real-time denoising.")]
            SpatialTemporal
        }
